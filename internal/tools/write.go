...
func odooWriteHandler(ctx context.Context, req *mcp.CallToolRequest, input WriteInput, deps Deps) (*mcp.CallToolResult, any, error) {
	start := time.Now()
	...
	// Call Odoo
	err = client.Write(ctx, input.Model, ids, values)
	if err != nil {
		auditResult(ctx, deps, audit.Entry{
			SessionID: sess,
			Tool:      "odoo_write",
		...
		return toolErrorf("write failed: %v", err), nil, nil
	}
